...
import com.yscope.log4j.appenders.compressedLogFileAppender.v2.compressionDictionary.LogtypeCompressionDictionary;
import com.yscope.log4j.appenders.compressedLogFileAppender.v2.compressionDictionary.VariableCompressionDictionary;
...
/**
 * v2 - Testing compression efficiency of "row-majored" CLP IR format
 * Result: as expected, "row-majored" performs worse compared to "column-major"
 * However, the final output file size could still be 3x smaller than zstd
 * Recommend to use column major when possible
 */
@Plugin(name = "CompressedLogFileV2", category = Core.CATEGORY_NAME, elementType = Appender.ELEMENT_TYPE, printObject = true)
public final class CompressedLogFileV2 extends AbstractAppender {
    ...
    private FileChannel encodedColumnFC;
    ...
    private MappedByteBuffer encodedColumnBuf;
    ...
    private CompressedLogFileV2(String name, Filter filter,
        ...
        try {
            ...
            encodedColumnFC = FileChannel.open(compressedLogDir.resolve("encodedColumn.bin"),
                    StandardOpenOption.CREATE, StandardOpenOption.WRITE, StandardOpenOption.READ);
            encodedColumnBuf = encodedColumnFC.map(FileChannel.MapMode.READ_WRITE, 0, bufferSize * 2);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    @PluginFactory
    public static CompressedLogFileV2 createAppender(
        ...
        return new CompressedLogFileV2(name, filter, compressedLogPatternLayoutContainer,
    ...
    private void compressIntermediateRepresentation() {
        try {
            ...
            zstdDirectBufferCompressingStream.compress(encodedColumnBuf.flip());
    ...
    private void truncateAllMemoryMappedFiles() {
        try {
            ...
            System.out.println("Flushing and truncating encodedColumn");
            truncateMemoryMappedFile(encodedColumnBuf, encodedColumnFC);
    ...
    private LongArrayList variableCache = new LongArrayList();
    ...
    public void append(LogEvent event) {
        ...
        try {
            ...
            encodedColumnBuf.putLong(event.getTimeMillis());
            ...
            while (parseNextVar(logMessage)) {
                ...
                if (var.parseLong()) {
                    ...
                    variableCache.add(var.longRepresentation);
                } else {
                    // Variable must be encoded
                    logtype.put((byte) 18);   // Delimiter for id type
                    variableCache.add(varDict.getVariableId(var));
                }
            }

            final LogtypeByteArrayView logtypeByteBufferWrapper = new LogtypeByteArrayView(logtype);
            encodedColumnBuf.putChar(logtypeDict.getLogtypeIdCompact(logtypeByteBufferWrapper));

            for (long l: variableCache) {
                encodedColumnBuf.putLong(l);
            }
            variableCache.clear();
