package com.yscope.log4j.appenders.compressedLogAggregationAppender.v1;
...
/**
 * v1 - Experiment with log aggregation stream appender
 *  - adapted from compressedLogFileAppenderV5
 * Note: must set the zstd compression level to 10 or greater to be able to handle sha256 digests efficiently
 */
@Plugin(name = "CompressedLogAggregationV1", category = Core.CATEGORY_NAME, elementType = Appender.ELEMENT_TYPE, printObject = true)
public final class CompressedLogAggregationV1 extends AbstractAppender {
    ...
    private CompressedLogAggregationV1(String name, Filter filter,
    ...
    @PluginFactory
    public static CompressedLogAggregationV1 createAppender(
        ...
        if (null == name) {
            LOGGER.error("No name provided for " + CompressedLogAggregationV1.class.getName());
            return null;
        }
        if (null == layout) {
        ...
        if (null == fileName) {
            LOGGER.error("No file name provided for " + CompressedLogAggregationV1.class.getName());
            return null;
        }

        return new CompressedLogAggregationV1(name, filter, compressedLogPatternLayoutContainer,
