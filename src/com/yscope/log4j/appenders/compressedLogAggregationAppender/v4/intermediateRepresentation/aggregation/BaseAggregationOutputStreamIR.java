...
import com.yscope.log4j.appenders.compressedLogAggregationAppender.v4.intermediateRepresentation.BufferedFileOutputStreamIR;
...
public abstract class BaseAggregationOutputStreamIR extends BufferedFileOutputStreamIR implements AggregationIR {
    ObjectOpenCustomHashSet<byte[]> logtypeDeduplicationHashSet;   // logtype and variable type can share the same set
    ObjectOpenCustomHashSet<byte[]> variableDeduplicationHashSet;   // logtype and variable type can share the same set
    ...
    public BaseAggregationOutputStreamIR(Path path, int compressionLevel) throws IOException {
        ...
        logtypeDeduplicationHashSet = new ObjectOpenCustomHashSet<>(new Sha256DigestHashStrategy<>());
        variableDeduplicationHashSet = new ObjectOpenCustomHashSet<>(new Sha256DigestHashStrategy<>());
    }
    ...
    public void putLogtype(ByteArrayViewDictionaryKey logtype) throws IOException, CloneNotSupportedException {
        ...
        if (logtypeDeduplicationHashSet.contains(digest)) {
            ...
            addLogtypeDeduplicationHashSetEntry(digest);
        }
    }
    ...
    public void putDictionaryVariable(ByteArrayViewDictionaryKey variable) throws IOException {
        ...
        if (logtypeDeduplicationHashSet.contains(digest)) {
            ...
            addVariableDeduplicationHashSetEntry(digest);
        }
    }

    private void addLogtypeDeduplicationHashSetEntry(byte[] digest) {
        // Insert a deep copy of digest into dictionary
        byte[] digestCopy = new byte[32];
        System.arraycopy(digest, 0, digestCopy, 0, 32);
        logtypeDeduplicationHashSet.add(digestCopy);
    }

    private void addVariableDeduplicationHashSetEntry(byte[] digest) {
        // Insert a deep copy of digest into dictionary
        byte[] digestCopy = new byte[32];
        System.arraycopy(digest, 0, digestCopy, 0, 32);
        variableDeduplicationHashSet.add(digestCopy);
    }
